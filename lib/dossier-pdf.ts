...
export type Dossier = {
  ...
  detail: string;
  reference: string;
  /* La date de la LIVRAISON, pas celle du jour. Le portail réassemble un envoi
     déjà fait : l'horodater à la date du téléchargement laisserait entendre
     que la traduction a été certifiée aujourd'hui, ce qui est faux et ce qui
     changerait à chaque clic. */
  emisLe?: string;
  pieces: PieceReliee[];
  certificat: { traducteur: string; adresse: string; contact: string };
  /* Mention portée en pied des pages que NOUS composons. Un texte plutôt
     qu'un booléen : un dossier fabriqué et un vrai dossier montré en
     démonstration n'ont pas la même chose à déclarer, et « not a student
     record » serait faux sur le second. */
...
export async function construireDossier(d: Dossier): Promise<Buffer> {
  ...
  y -= 13;
  txt(50, y, `Issued ${d.emisLe ?? new Date().toISOString().slice(0, 10)}`, 9, normale, GRIS);
  y -= 30;
