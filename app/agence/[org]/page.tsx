...
import { fusionnerReglees, regleesParSportif } from '@/lib/agence/suivi';
...
export default async function Sportifs({ params }: { params: Promise<{ org: string }> }) {
  ...
  const reglees = await regleesParSportif(ctx.org.id);
  const lignes = sportifsDe(slug)
    .map((c) => fusionnerReglees(c, reglees.get(c.id) ?? []))
